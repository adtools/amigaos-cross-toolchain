/*
 * BFD back-end for Commodore-Amiga AmigaOS binaries. Copyright (C) 1990-1994
 * Free Software Foundation, Inc. Contributed by Leonard Norrgard.  Partially
 * based on the bout and ieee BFD backends and Markus Wild's tool hunk2gcc.
...
/*
 * The following are gross hacks that need to be fixed.  The problem is that
 * the linker unconditionally references these values without going through
 * any of bfd's standard interface.  Thus they need to be defined in a bfd
 * module that is included in *all* configurations, and are currently in
 * bfd.c, otherwise linking the linker will fail on non-Amiga target
 * configurations.
 */
/*
...
amiga_object_p (bfd * abfd)
{
  ...
  /*
   * Can't fail and return (but must be declared boolean to suit other
   * bfd requirements).
   */
...
static asection *
amiga_get_section_by_hunk_number (bfd * abfd, long hunk_number)
{
...
/*
 * BFD doesn't currently allow multiple sections with the same name, so we
 * try a little harder to get a unique name.
 */
asection *
amiga_make_unique_section (bfd * abfd, CONST char *name)
{
  asection *section;
...
/*
 * Write a string padded to 4 bytes and preceded by it's length in long words
 * ORed with <value>
 */
static boolean
 ...
 * we try to allocate space for this section, if  this has not already been
 * done Then we set the memory area to the contents
 */
static boolean
...
DEFUN (amiga_sizeof_headers, (ignore_abfd, ignore),
       bfd * ignore_abfd AND boolean ignore)
{
     ...
     bfd_reloc_code_real_type code;
{
  switch (code)
    ...
    case BFD_RELOC_8_PCREL:
      return &howto_hunk_reloc8;
    case BFD_RELOC_16_PCREL:
      return &howto_hunk_reloc16;
    case BFD_RELOC_CTOR:
    case BFD_RELOC_32_PCREL:
      return &howto_hunk_reloc32;
    case BFD_RELOC_8:
      return &howto_hunk_reloc8;
    case BFD_RELOC_16:
      return &howto_hunk_reloc16;
...
/*
 * There is no armap in the amiga libraries, so we fill carsym entries one by
 ...
 * a.out files. In addition, we use an own final_link routine, which is
